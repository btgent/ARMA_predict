guess_arima <- function(min_pq,max_pq,p,q,training_size) {
  ...
  training <- sim[1:training_size]
  predict_d <- sim[(training_size+1):(training_size+10)]
  ...
  for (i in min_pq:max_pq) {
    for (j in min_pq:max_pq) {
      m_aic = try(AIC(arima(x = training, order = c(i, 0, j))),silent = T)
  ...
  if (min_aic < 10^6) {
    estimate <- predict(arima(training, order=c(guess_p, 0,guess_q)), n.ahead=10)
    mse1 <- mean((predict_d[1] - estimate$pred[1])^2)
    mse2 <- mean((predict_d[1:2] - estimate$pred[1:2])^2)
    mse5 <- mean((predict_d[1:5] - estimate$pred[1:5])^2)
    mse10 <- mean((predict_d[1:10] - estimate$pred[1:10])^2)
    
    correct_pq = ((p==guess_p)&&(q==guess_q))
